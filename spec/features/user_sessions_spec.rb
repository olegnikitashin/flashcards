...
describe "User Sessions", type: :feature do
  ...
  describe '#create' do
    it 'will Log In a user' do
      visit login_url
      fill_in I18n.t('home.user_sessions.form.email'), with: user.email
    ...
    it 'will not Log In a user with wrong password' do
      visit login_url
      fill_in I18n.t('home.user_sessions.form.email'), with: user.email
      fill_in I18n.t('home.user_sessions.form.password'), with: 'foobar1'
      click_button I18n.t('home.user_sessions.form.login_path')
      expect(page).to have_content I18n.t('home.user_sessions.create.login_failed')
    end
  end
  describe '#destroy' do
    before do
    ...
    it 'will Log Out a user' do
      ...
      expect(page).to have_content I18n.t('dashboard.user_sessions.destroy.logout_success')
    end
  end
  describe 'Restricted access' do
    it 'will prevent from getting access without login' do
      ...
      paths.each do |path|
        visit path
        expect(page).to have_content I18n.t('dashboard.home.welcome.welcome_message')
      end
    end
  end
end
