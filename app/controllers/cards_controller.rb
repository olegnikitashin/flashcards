class CardsController < ApplicationController
  ...
  def create
    ...
    if @card.save
      flash[:notice] =  t('card.success_create', card: @card.original_text)
      redirect_to @card
    else
      render 'new'
    end
  ...
  def update
    if @card.update(card_params)
      flash[:notice] = t('card.success_update', card: @card.original_text)
      redirect_to @card
    else
      render :edit
    end
  ...
  def destroy
    @card.destroy
    flash[:notice] = t('card.success_delete', card: @card.original_text)
